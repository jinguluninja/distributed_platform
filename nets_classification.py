...
def custom_model(layer, is_training, class_num, batch_size, dropout=0.0, name="custom"):
    """
    Fill in this function for custom model
    INPUTS:
    - layer: (tensor.4d) input tensor.
    - is_training: (bool) are we in training 
    - class_num: (int) number of label classes
    - batch_size: (int) batch size
    - dropout: (float) probability to drop during dropout.
    - name: (str) the name of the network

    MUST RETURN OUTPUT (see below models for examples)
    """
    pass

def linear_model(layer, is_training, height, width, channels, class_num, batch_size, dropout=0.0, name="linear"):
    """
    Fill in this function for custom model
    INPUTS:
    - layer: (tensor.4d) input tensor.
    ...
    MUST RETURN OUTPUT (see below models for examples)
    """
    layer = tf.reshape(layer, shape=[-1, height*width*channels])
...
def Le_Net(layer, is_training, class_num, batch_size, dropout=0.0, name="Le_Net"):
    """
...
def GoogLe_Net(layer, is_training, class_num, batch_size, dropout=0.0, name="GoogLe_Net"):
    """
...
def Res_Net(layer, is_training, class_num, batch_size, dropout=0.0, name="Res_Net"):
    """
    This is a 50 layer Res Net.
    INPUTS:
    - layer: (tensor.4d) input tensor.
    - output_size: (int) number of classes we're predicting
    - dropout: (float) probability to drop during dropout.
